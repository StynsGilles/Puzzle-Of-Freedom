...
float3 CalculateSpecularBlinn(float3 viewDirection, float3 normal, float2 texCoord)
{
    ...
    return specularIntensity * specColor;
}
...
float3 CalculateSpecularPhong(float3 viewDirection, float3 normal, float2 texCoord)
{
    ...
    float shininess = specularIntensity * gShininess;
...
float3 CalculateSpecular(float3 viewDirection, float3 normal, float2 texCoord)
{
    if (gUseSpecularBlinn && gUseSpecularPhong)
        return CalculateSpecularBlinn(viewDirection, normal, texCoord) + CalculateSpecularPhong(viewDirection, normal, texCoord);
...
{
    float diffuseLightValue = dot(normal, -gLightDirection);
    diffuseLightValue = max(diffuseLightValue, 0);
    if (gUseTextureDiffuse)
        return gTextureDiffuse.Sample(gTextureSampler, texCoord).rgb * diffuseLightValue;
    return gColorDiffuse.rgb * diffuseLightValue;
}
...
float3 CalculateFresnelFalloff(float3 normal, float3 viewDirection, float3 environmentColor)
{
    ...
    {
        float3 fresnel = ((1 - saturate(dot(normal, viewDirection)))) * gFresnelMultiplier;
        float3 fresnelMask = pow((1 - saturate(dot(float3(0, -1, 0), normal))), gFresnelHardness);
        return fresnel * fresnelMask * environmentColor * gColorFresnel.xyz;
    }
...
float3 CalculateEnvironment(float3 viewDirection, float3 normal)
{
    if (gUseEnvironmentMapping)
    {
        ...
        reflectedColor = reflectedColor * gReflectionStrength;
        refractedColor = refractedColor * gRefractionStrength;
        return float4(reflectedColor + refractedColor, 1.0f);
    }
    return float3(0, 0, 0);
}

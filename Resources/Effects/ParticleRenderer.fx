...
void CreateVertex(inout TriangleStream<GS_DATA> triStream, float3 pos, float2 texCoord, float4 col)
{
    ...
    geomData.Position = mul(float4(pos, 1.f), gWorldViewProj);
...
void MainGS(point VS_DATA vertex[1], inout TriangleStream<GS_DATA> triStream)
{
    ...
    float sizeDiv2 = size / 2.f;
    topLeft = float3(-sizeDiv2, sizeDiv2, origin.z);
    topRight = float3(sizeDiv2, sizeDiv2, origin.z);
    bottomLeft = float3(-sizeDiv2, -sizeDiv2, origin.z);
    bottomRight = float3(sizeDiv2, -sizeDiv2, origin.z);
    ...
    topLeft = float3(mul((float2) topLeft, uvRotation), 0.f);
    topRight = float3(mul((float2) topRight, uvRotation), 0.f);
    bottomLeft = float3(mul((float2) bottomLeft, uvRotation), 0.f);
    bottomRight = float3(mul((float2) bottomRight, uvRotation), 0.f);
